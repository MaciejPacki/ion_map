...
def azimuth_elevation(xyz_sat, xyz_rec):
    ...
    n_hat = numpy.array([- math.cos(l) * math.sin(b),- math.sin(l) * math.sin(b), math.cos(b)])
    u_hat = numpy.array([math.cos(l) * math.cos(b), math.sin(l) * math.cos(b), math.sin(b)])
...
def ipp(blh_site, az_el, ionosphere_h):
    ...
    b_ipp = math.asin(math.sin(b) * math.cos(psi) + math.cos(b) * math.sin(psi) * math.cos(az))
